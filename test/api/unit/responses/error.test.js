...
describe('error response', () => {
  ...
  before(function () {
    this.sails.get('/error-test/db-problem', (req, res) => {
      res.notOk(new ExceptionService.DatabaseError())
    })
    this.sails.get('/error-test/bad-request', (req, res) => {
      res.notOk(new ExceptionService.BadRequest('hmmm'))
    })
    this.sails.get('/error-test/forbidden', (req, res) => {
      res.notOk(new ExceptionService.Forbidden())
    })
    this.sails.get('/error-test/not-found', (req, res) => {
  ...
  it('should not include data in the response when environment is production', function (next) {
    ...
    this.request.get('/error-test/db-problem').expect(500).end((err, res) => {
